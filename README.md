...
This repository contains various project submissions for a CS 12 course at
ADSS.
...
The `qmd` file extension indicates a file uses [Quarto][quarto-hm] flavoured
Markdown. These may use additional feature regular Markdown doesn't have, and
use YAML-frontmatter to declare metadata.

To compile `qmd` files into HTML format, you must first install Quarto. To
install on Ubuntu, download the [deb file][quarto-dl], and install with `dpkg`:
...
[quarto-hm]: https://quarto.org/
[quarto-dl]: https://quarto.org/docs/download/
